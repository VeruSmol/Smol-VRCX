 ...
 * @param {string} link
 */
function normalizeVrcdnLink(link) {
  ...
  try {
    ...
    // [smol] - keep original URL if decodeURI fails
  }

  // [smol] - msyvh VRCDN stream links and convert them to preview links
  const streamMatch = decoded.match(
    /^(rtmp|rtsp|rtspt|ws|wss|http|https):\/\/stream\.vrcdn\.live\/live\/(.+)$/i,
  );
  ...
  let vrcdnUser = streamMatch[2];

  // [smol] - strip known stream suffixes
  vrcdnUser = vrcdnUser.replace(/\.(live\.)?(ts|flv|mp4)$/i, "");

  return `https://panel.vrcdn.live/preview/${vrcdnUser}`;
}
...
function openExternalLink(link) {
  const normalizedLink = normalizeVrcdnLink(link); // [smol] - vrcdn link fixer

  const searchStore = useSearchStore();
  if (searchStore.directAccessParse(normalizedLink)) {
    return;
  }
  ...
  modalStore
    ...
    .then(({ ok, reason }) => {
      if (reason === "cancel") {
        copyToClipboard(normalizedLink, "Link copied to clipboard!");
        return;
      }
      if (ok) {
        AppApi.OpenLink(normalizedLink);
        return;
      }
    });
}
