...
/** [smol]
 * Format remaining seconds as a compact countdown.
 * Examples: 00:43, 12:08, 1:05:42
 * @param {number} remainingSeconds
 * @returns {string}
 */
export function formatCountdownSeconds(remainingSeconds) {
  const safe = Math.max(0, Math.ceil(Number(remainingSeconds || 0)));
  const hours = Math.floor(safe / 3600);
  const minutes = Math.floor((safe % 3600) / 60);
