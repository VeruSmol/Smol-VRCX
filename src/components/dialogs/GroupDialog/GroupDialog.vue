<template>
  <div class="w-223">
    <DialogHeader class="sr-only">
      <DialogTitle>{{
        groupDialog.ref?.name || t("dialog.group.info.header")
      }}</DialogTitle>
      <DialogDescription>
        {{
        ...
        >
          <div class="group-header" style="flex: 1">
            ...
            <span class="group-discriminator x-grey mr-1.5 font-mono text-xs">
              {{ groupDialog.ref.shortCode }}.{{
                groupDialog.ref.discriminator
              }}
            </span>
            <TooltipWrapper
            ...
            <div class="group-tags">
              ...
              <Badge
                v-if="
                  groupDialog.ref.myMember && groupDialog.ref.myMember.bannedAt
                "
                variant="outline"
                style="margin-right: 6px; margin-top: 6px"
              >
          ...
          <div class="ml-2 mt-12">
            ...
            <template
              v-else-if="
                groupDialog.ref.myMember?.membershipStatus === 'requested'
              "
            >
              <TooltipWrapper
                side="top"
            ...
            <template
              v-else-if="
                groupDialog.ref.myMember?.membershipStatus === 'invited'
              "
            >
              <TooltipWrapper
                side="top"
              ...
              <DropdownMenuContent>
                ...
                <template v-if="groupDialog.inGroup">
                  <template v-if="groupDialog.ref.myMember">
                    ...
                    <DropdownMenuItem
                      v-if="
                        groupDialog.ref.myMember.isSubscribedToAnnouncements
                      "
                      @click="
                        groupDialogCommand('Unsubscribe To Announcements')
                      "
                    >
                    ...
                    <template
                      v-if="
                        groupDialog.ref.myMember &&
                        groupDialog.ref.privacy === 'default'
                      "
                    >
                      <DropdownMenuSeparator />
                      <DropdownMenuItem
                      ...
                      >
                        ...
                        <Check
                          v-if="
                            groupDialog.ref.myMember.visibility === 'visible'
                          "
                          class="size-4"
                        />
                      ...
                      >
                        ...
                        <Check
                          v-if="
                            groupDialog.ref.myMember.visibility === 'friends'
                          "
                          class="size-4"
                        />
                      ...
                      >
                        ...
                        <Check
                          v-if="
                            groupDialog.ref.myMember.visibility === 'hidden'
                          "
                          class="size-4"
                        />
                ...
                <template v-else>
                  ...
                  <DropdownMenuItem
                    v-else
                    @click="groupDialogCommand('Block Group')"
                  >
    ...
    <GroupPostEditDialog
      :dialog-data="groupPostEditDialog"
      :selected-gallery-file="selectedGalleryFile"
    />
  </div>
</template>
...
const { updateGroupPostSearch, showGroupMemberModerationDialog } =
  useGroupStore();
    ...
    .then((args) => {
      if (groupDialog.value.visible && groupDialog.value.id === args.groupId) {
        updateGroupDialogData({
          ...groupDialog.value,
          ref: {
            ...groupDialog.value.ref,
            isRepresenting: args.params.isRepresenting,
          },
        });
        getGroupDialogGroup(groupId);
      }
      refetchActiveEntityQuery(
        queryKeys.representedGroup(currentUser.value.id),
      );
    });
}
