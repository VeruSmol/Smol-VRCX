        ...
        <div class="flex items-center w-full h-full px-2">
          ...
          >
            ...
            >
              ...
              <HoverCardContent
                v-if="
                  gameStore.isGameRunning && userStore.currentUser.$online_for
                "
              ...
              <HoverCardContent
                v-else-if="
                  !gameStore.isGameRunning &&
                  gameStore.lastSessionDurationMs > 0
                "
                class="w-auto min-w-[160px] px-3 py-2"
                side="top"
                align="start"
            ...
            <HoverCard
              v-if="visibility.servers"
              v-model:open="serversHoverOpen"
            >
              <HoverCardTrigger as-child>
                <TooltipWrapper
                ...
                >
                  ...
                  >
                    <span
                      class="inline-block size-2 rounded-full shrink-0 bg-status-online"
                    />
                    <span class="text-foreground text-[11px]">{{
          ...
          <TooltipWrapper
            v-if="instanceWatcherActive"
            :content="instanceWatcherTooltip"
            side="top"
          >
            <div
              class="flex items-center gap-1 px-2 h-[22px] whitespace-nowrap border-r border-border"
            >
              <span class="text-[10px] text-foreground">{{
                instanceWatcherActionText
              }}</span>
              <span class="text-[10px] text-foreground">{{
                t("status_bar.instance_watcher_in_group")
              }}</span>
              <button
                type="button"
                class="text-[10px] text-foreground underline underline-offset-2 hover:text-primary max-w-[180px] truncate"
                @click.stop="openWatchedGroupInstances"
              >
                {{ instanceWatcherGroupName }}
              </button>
              <span class="text-[10px] text-foreground">{{
                t("status_bar.instance_watcher_for_time")
              }}</span>
              <span class="text-[10px] text-foreground">{{
                instanceWatcherRemainingText
              }}</span>
            </div>
          </TooltipWrapper>
          <!-- Right section -->
          <div
            class="flex items-center shrink-0 ml-auto [&>*:last-child]:border-r-0 [&>*:last-child]:pr-0.5"
          >
                ...
                <PopoverContent class="w-[280px]" side="top" align="center">
                  <div class="flex flex-col gap-2 p-1">
                    ...
                    <Select
                      ...
                      @update:modelValue="
                        (offset) => updateClockTimezone(idx, offset)
                      "
                    >
                      ...
                      <SelectContent class="max-h-60">
                        <SelectGroup>
                          ...
                          >
                            <div
                              class="flex w-full items-center justify-end font-mono"
                            >
                              {{ opt.label }}
                            </div>
                          </SelectItem>
            ...
            <TooltipWrapper
              v-if="visibility.uptime"
              :content="t('status_bar.app_uptime')"
              side="top"
            >
              <div
                class="flex items-center gap-1 px-2 h-[22px] whitespace-nowrap border-r border-border"
...
const gameSessionDetailText = computed(() => {
  if (!gameStore.isGameRunning || !userStore.currentUser.$online_for)
    return "-";
  const elapsed = now.value - userStore.currentUser.$online_for;
...
// --- Instance watcher banner ---
const instanceWatcherActive = computed(() => {
  void now.value;
  return (
    getSmolWatchNewInstances() &&
    Boolean(getSmolWatchedGroupId()) &&
    getSmolAutoOpenRemainingSeconds() > 0
  );
});

const instanceWatcherGroupId = computed(() => {
  void now.value;
  return getSmolWatchedGroupId();
});

const instanceWatcherGroupName = computed(() => {
  void now.value;
  return getSmolWatchedGroupName();
});

const instanceWatcherGroupIsMember = computed(() => {
  void now.value;
  return getSmolWatchedGroupIsMember();
});

const instanceWatcherRemainingSeconds = computed(() => {
  void now.value;
  return getSmolAutoOpenRemainingSeconds();
});

const instanceWatcherRemainingText = computed(() => {
  return formatCountdownSeconds(instanceWatcherRemainingSeconds.value);
});

const instanceWatcherActionText = computed(() => {
  return instanceWatcherGroupIsMember.value
    ? t("status_bar.instance_watcher_opening")
    : t("status_bar.instance_watcher_opening_public");
});

const instanceWatcherTooltip = computed(() => {
  if (!instanceWatcherActive.value) return "";

  if (!instanceWatcherGroupIsMember.value) {
    return t("status_bar.instance_watcher_tooltip_public_only", {
      group: instanceWatcherGroupName.value,
    });
  }

  return t("status_bar.instance_watcher_tooltip", {
    action: instanceWatcherActionText.value,
    group: instanceWatcherGroupName.value,
    remaining: instanceWatcherRemainingText.value,
  });
});
useIntervalFn(() => {
 ...
 *
 */
function openWatchedGroupInstances() {
  const groupId = instanceWatcherGroupId.value;
  if (!groupId) return;

  // This is the only navigation helper visible in the files you sent.
  // If you already have a dedicated "open instances tab" helper elsewhere,
  // replace this function body with that call.
...
function formatClock(clock) {
  try {
    const current = dayjs(now.value).utcOffset(
      normalizeUtcHour(clock.offset) * 60,
    );
