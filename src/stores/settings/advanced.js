...
export const useAdvancedSettingsStore = defineStore("AdvancedSettings", () => {
  ...
  function setSmolInstancePollSeconds(value) {
    smolInstancePollSeconds.value = saveSmolInstancePollSecondsConfig(value);
  }

  // [smol] - save advanced setting for keeping watcher alive after dialog close
  ...
  async function purgeAvatarFeedData(days) {
    ...
    try {
      ...
      toast.dismiss(msgBox);
      toast.error(
        t("view.settings.advanced.advanced.database_cleanup.purge_failed", {
          error: err,
        }),
      );
    } finally {
