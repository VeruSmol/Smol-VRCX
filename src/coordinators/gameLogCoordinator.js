...
/**
 * [smol] - vrcdn link -> preview panel link fixer
 */
function normalizeVideoPlayerUrl(url) {
  ...
  try {
    ...
    // [smol] - Keep original URL if decodeURI fails for some reason
  }

  let vrcdnUser = null;

  // [smol] - match rtmp, rtspt, ws, wss, http, and https
  const streamMatch = decoded.match(
    /^(rtmp|rtspt|ws|wss|http|https):\/\/stream\.vrcdn\.live\/live\/(.+)$/i,
  );
  ...
  vrcdnUser = streamMatch[2];

  // [smol] - strip known media suffixes from the stream key
...
export function addGameLogEntry(gameLog, location) {
  ...
  switch (gameLog.type) {
    ...
    case "portal-spawn":
      ...
      break;
    case "video-play":
      gameLog.videoUrl = normalizeVideoPlayerUrl(gameLog.videoUrl);
      console.log("[smol] normalized videoUrl:", gameLog.videoUrl);
